...
def mostrar_producto_completo(producto):
    ...
    precio_formateado = f"{producto['Precio']:,.0f}".replace(",", ".")  # Formatear el precio
    st.markdown(f"<span style='font-size: 28px; font-weight: bold;'>Código: {producto['Codigo']} | Precio: ${precio_formateado} | Stock: {producto['Stock']}</span>", unsafe_allow_html=True)  # Mostrar código, precio y stock
...
def mostrar_lista_productos(df, pagina, productos_por_pagina=10):
    ...
    for i, producto in productos_pagina.iterrows():
        ...
        with col2:
            ...
            precio_formateado = f"{producto['Precio']:,.0f}".replace(",", ".")  # Formatear el precio
            st.markdown(f"Código: {producto['Codigo']} | Precio: ${precio_formateado} | <span style='color: {stock_color};'>STOCK: {producto['Stock']}</span>", unsafe_allow_html=True)  # Cambiar color del stock
...
if busqueda == '' and not (ver_por_categorias or ordenar_por_novedad or sugerir_por_rubro):
    st.image('bot (8).png', width=480, use_column_width='auto')  # Mostrar imagen solo si el buscador está vacío y no hay casillas marcadas

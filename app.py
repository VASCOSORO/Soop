...
def mostrar_producto_completo(producto):
    ...
    st.markdown(f"<span style='font-size: 28px; font-weight: bold;'>Código: {producto['Codigo']} | Precio: ${producto['Precio Face']} | Stock: {producto['Stock']}</span>", unsafe_allow_html=True)  # Mostrar código, precio y stock
...
def mostrar_lista_productos(df, pagina, productos_por_pagina=10):
    ...
    for i, producto in productos_pagina.iterrows():
        ...
        with col2:
            ...
            st.markdown(f"Código: {producto['Codigo']} | Precio: ${producto['Precio Face']} | <span style='color: {stock_color};'>STOCK: {producto['Stock']}</span>", unsafe_allow_html=True)  # Cambiar color del stock
